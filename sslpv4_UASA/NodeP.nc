...
{	
	...
	command error_t Node.addService(char *serv,char *scop,char *url,uint16_t lifetime)
	{
	...
	command error_t Node.removeService(char *service,char *scope,char *url)
	{
		uint8_t index;
		servicelocation_entry location_entry;
		index=findService(service);
	...
	command uint8_t Node.findService(char *service,char *scope)
	{
		int index;
		if(call Node.getSAState())
		{
	...
	command char * Node.getServices(char *scope,uint16_t len)
	{
		if(call Node.getSAState())
		{
	...
	command bool Node.servicesPresent(char *scope)
	{
		if(call Node.getSAState())
		{
			uint8_t i;
	...
	command error_t Node.getServiceURL(char *servicetype,char *scope,char *url)
	{
